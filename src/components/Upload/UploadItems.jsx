...
const UploadItems = ({ items = [], onChange, fields, setActiveField }) => {
  ...
  // --- FIXED: Add Item Logic ---
  const addNewItem = () => {
    // 1. Create a clean row mapping the tech names to empty strings
    const newItem = { id: Date.now().toString() };
    fields.forEach(f => {
      newItem[f.fieldTechName] = "";
    });
    ...
    // 3. Set editing to the INDEX of the newly added item (not the ID)
    setEditingRow(updatedItems.length - 1);
  };

  const updateItem = (indexToUpdate, fieldName, value) => {
    const updatedItems = items.map((item, index) =>
      index === indexToUpdate ? { ...item, [fieldName]: value } : item
    );
    onChange(updatedItems);
  };

  // --- FIXED: Delete Item Logic ---
  const deleteItem = (indexToRemove) => {
    // Filter out the item using the correct callback index
    onChange(items.filter((_, index) => index !== indexToRemove));
    if (editingRow === indexToRemove) setEditingRow(null);
  };
  ...
  const renderCellContent = (item, column, itemindex) => {
    ...
    if (!isEditing) {
      if (column.fieldTechName?.toLowerCase().includes("expense") && value) {
        return <Badge variant="outline">{value}</Badge>;
      }
      if (column.fieldTechName?.toLowerCase().includes("currency") && value) {
        return <Badge variant="secondary">{value}</Badge>;
      }
    ...
    // --- FIXED TYPOS: Capitalized fieldType everywhere ---
    if (column.fieldType === "String") {
      return (
        <Input
          value={value}
          onChange={(e) => updateItem(itemindex, column.fieldTechName, e.target.value)}
          onClick={() => setActiveField && setActiveField(column.fieldTechName)}
          className="w-full"
        />
      );
    }
    ...
    if (column.fieldType === "Number") {
      return (
        <Input
          type="number"
          value={value}
          onChange={(e) => updateItem(itemindex, column.fieldTechName, parseFloat(e.target.value) || 0)}
          onClick={() => setActiveField && setActiveField(column.fieldTechName)}
          className="w-full"
        />
      );
    ...
    if (column.fieldType === "Dropdown" && column.dropdown) {
      return (
        <Select
          value={value}
          onValueChange={(val) => updateItem(itemindex, column.fieldTechName, val)}
        >
          <SelectTrigger className="w-full">
            <SelectValue placeholder="Select" />
          </SelectTrigger>
          ...
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow className="bg-expense-table-header">
                  {columns.map((column) => (
                    <TableHead key={column.id} className="font-medium whitespace-nowrap">
                      {column.name}
                    </TableHead>
                  ))}
              ...
              <TableBody>
                {items.map((item, itemindex) => (
                  <TableRow key={item.id || itemindex} className="hover:bg-expense-row-hover transition-colors">
                    {columns.map((column, index) => (
                      <TableCell key={`column-${column.id}-${index}`} className="p-3">
                        {renderCellContent(item, column, itemindex)}
                      </TableCell>
                    ))}
                    <TableCell className="p-3">
                      <div className="flex gap-1">
                        {editingRow === itemindex ? (
                          ...
                          <Button size="sm" variant="outline" onClick={() => setEditingRow(itemindex)}>
                            Edit
                          </Button>
                        )}
                        <Button
